...
#
    ...
    MAKE_OBJECT:=obj
    MAKE_SOURCE:=src
    MAKE_BNPATH:=/media/dhlab-linux/lib
    MAKE_HDPATH:=/media/dhlab-linux/usr/include
    MAKE_CMCOPY:=cp
    MAKE_CMRMFL:=rm -f
    MAKE_CMRMRF:=rm -rf
    MAKE_CMMKDR:=mkdir -p
    ...
    BUILD_FLAGS:=-Wall -funsigned-char -pedantic -O3 -D _BSD_SOURCE
    BUILD_LINKD:=-lm -lpthread
    BUILD_SUBMD:=
